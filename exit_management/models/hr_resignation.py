...
class HrResignation(models.Model):
    ...
    exit_interview_feedback = fields.Text(string="Exit Interview Feedback", help='Specify reason for leaving the company')
    ...
    @api.onchange('employee_id','date_of_resignation','emp_relieving_date','termination_date','date_of_absconding','retirement_date','date_of_death')
    @api.depends('employee_id','date_of_resignation','emp_relieving_date','termination_date','date_of_absconding','retirement_date','date_of_death')
    ...
    def compute_sheet(self):
        ...
        for slip in resignation_slip:
            ...
            lines = [(0, 0, line) for line in slip._get_payslip_lines(contract_ids=payslip.employee_id.contract_id.id, payslip_id=payslip.id)]
    ...
    def check_pending_month_salary(self):
        payslips = self.env['hr.payslip'].search([('employee_id', '=', self.employee_id.id), ('state', 'not in', ('paid', 'cancel'))], order="id")
        data = {}
        i = 0
        name = {}
        x = 0
    ...
    def get_total_leave_days(self):
        ...
        leave_enc_days = 0
        allocation = self.env['hr.leave.allocation'].search([('employee_id','=',emp.id),('holiday_status_id.is_encashable','=',True)])
        alloc_days = 0
        taken_days = 0
    ...
    def gratuity_calculation(self):
        return self.calculate_gratuity
