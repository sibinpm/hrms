...
class ItReturns(models.Model):
    ...
    date_to = fields.Date("To Date", default=date.today().replace(day=1) + relativedelta(months=1) - relativedelta(days=1))
    ...
    tds_computation_month = fields.Selection(selection='list_all_months', string='TDS Month', default=datetime.today().strftime("%b-%Y"), required=1)
    ...
    def _onchange_employee_id(self):
        for rec in self:
            ...
            pay_slip_id = self.env['hr.payslip'].search([('employee_id', '=', rec.employee_id.id)], order='id desc', limit=1)
        ...
        for rec in self:
            ...
            for dates in date_list:
                payslip = self.env['hr.payslip'].search([('employee_id', '=', rec.employee_id.id),
                                                         ('date_from', '=', dates),
                                                         ('date_to', '=', dates + relativedelta(months=1) - relativedelta(days=1))])
        ...
        for rec in self:
            ...
            if pay_slip_id:
                ...
                if rec.tax_resign_type.name == 'Old Regime':
                        ...
                        for lines in rec.section_10_line_ids:
                            if 'House Rent Allowance' in lines.section_10.name:
                                if rec.employee_id.rent_paid:
                                    ...
                                    amount_from_basic = (pay_slip_id.basic_salary * (rec.employee_id.city_type_conf.percent / 100))
                                    rent_paid = (rec.employee_id.rent_paid * 12) - (pay_slip_id.basic_salary * (10 / 100))
                                    ...
                                    rec.update({
                                        'computed_line_ids': [(0, 0, {'name': lines.section_10.name,
                                                                      ...
                                                                      'color': 'brown',
                                                                      'amount_total': min(actual_hra_received, amount_from_basic, rent_paid)
                                                                      })],
                                    })
                                    lines.update({
                                        'amount': min(actual_hra_received, amount_from_basic, rent_paid),
                    ...
                    if net_amount:
                        rec.update({
                            'computed_line_ids': [(0, 0, {'name': 'Total amount of salary received from current employer',
                                                          'amount': net_amount,
                    ...
                    if sec24_total:
                        rec.update({
                            'computed_line_ids': [(0, 0, {'name': 'Income (or admissible loss) from house property reported by employee offered for TDS',
                                                          'amount': sec24_total,
                                                          'color': 'bold',
                    ...
                    if oth_in_total and sec24_total:
                        rec.update({
                            'computed_line_ids': [(0, 0, {'name': 'Total amount of other income reported by the employee',
                    ...
                    if net_amount:
                        ...
                        for slab_lines in self.tax_resign_type.line_ids:
                            if (net_amount > slab_lines.higher_limit):
                                applied_tax += round(slab_lines.higher_limit - slab_lines.lower_limit) * (slab_lines.tax_per / 100)
                    ...
                    pay_slip = self.env['hr.payslip'].search([('employee_id', '=', rec.employee_id.id),
                                                              ('date_from', '<=', datetime.strptime("2-" + month, "%d-%b-%Y")),
                                                              ('date_to', '>=', datetime.strptime("2-" + month, "%d-%b-%Y"))], limit=1)
                    if tds_id:
                        tds_id.update({
                            ...
                            'total_amount': round((applied_tax - recovered_amounts.get('i_tax')) / future_months) + round((sur_tax - recovered_amounts.get('surcharge'))/future_months) + round((cess_tax - recovered_amounts.get('cess'))/future_months),
    ...
    def report_recovery_for_the_month(self):
        emp_tds_id = self.env['employee.tds'].search([('employee_id', '=', self.employee_id.id),('date', '=', datetime.today().replace(day=1))])
        if emp_tds_id:
            return {
